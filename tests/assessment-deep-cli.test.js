...
const directory = mkdtempSync(join(tmpdir(), 'sudoku-deep-'));
function run(...args) {
    const result = spawnSync(process.execPath, [join(repoRoot, 'scripts/assess-deep.js'), ...args],
        { cwd: directory, encoding: 'utf8', timeout: 30000, maxBuffer: 8 * 1024 * 1024 });
    if (result.error) throw result.error;
    return result;
}
afterAll(() => rmSync(directory, { recursive: true, force: true }));
describe('maintenance assessment CLI', () => {
    it('reproduces reports, records its policy and refuses to overwrite earlier results', () => {
        ...
        expect(report.policy.version).toBe('human-v3.0');
        expect(report.summary.solved + report.summary.unresolved).toBe(12);
        expect(report.provenance.sourceHashes['dynamic-chains.js']).toMatch(/^[a-f0-9]{64}$/);
    ...
    it('accepts an unseen board and provides a trace only when requested', () => {
        const board = '000030029700009400080600007060000090030500000002000030500860070010000008300010000'.replace(/[1-9]/g, d => String(10 - Number(d)));
