import { REASONING_FAMILIES } from '../../reasoning.js';
...
export function summarizeAssessments(records) {
    ...
    const unresolved = records.filter(record => ['unresolved', 'budget-exhausted'].includes(record.human.status));
    ...
    for (const difficulty of [...new Set(records.map(r => r.difficulty))]) {
        ...
        tiers[difficulty] = {
            ...
            unresolved: group.filter(r => ['unresolved', 'budget-exhausted'].includes(r.human.status)).length,
    ...
    add(records.find(r => !r.production.solved && r.human.status === 'solved'), 'persistent-candidate v1 assessment stalled');
    add(records.find(r => r.production.solved && r.human.status !== 'solved'), 'regression/disagreement to investigate before ordering');
    add(records.find(r => r.difficulty === 'easy' && ['unresolved', 'budget-exhausted'].includes(r.human.status)), 'existing Easy label requires review');
...
export function renderAssessmentReport(report) {
    ...
    const lines = [
        ...
        `through the preserved persistent-candidate v1 assessment. **${s.newlySolved} additional finishes;`,
        ...
        `Work-budget exhaustion was reported for ${report.records.filter(r => r.human.status === 'budget-exhausted').length} boards; other unfinished paths stalled within the configured techniques/depth.`, '',
    ...
    lines.push('## Reproduction and provenance', '',
        ...
        '```bash',
        `node scripts/assess-deep.js ${p.selection === 'all' ? '--all' : `--sample=${p.samplePerTier}`} --out=e2e-results/deep-v3-rerun.json --report=e2e-results/deep-v3-rerun.md`,
        '```', '',
