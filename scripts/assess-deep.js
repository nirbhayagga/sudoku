...
import { assessHumanPuzzle as assessV1 } from '../human-rating.js';
import { parsePuzzleText } from '../format.js';
import { assessPuzzleDeep as assessHumanPuzzle } from '../deep-assessment.js';
import { REASONING_FAMILIES, REASONING_RULES } from '../reasoning.js';
import { EXTENDED_VERSION as HUMAN_RATING_VERSION, ANALYSIS_PROFILES } from '../extended-reasoning.js';
const HUMAN_FAMILIES = [...REASONING_FAMILIES, 'dynamic-chains'];
const HUMAN_TECHNIQUE_ORDER = [...REASONING_RULES.map(rule => rule.type), 'dynamic-forcing-chain', 'forcing-convergence'];
import { summarizeAssessments, renderAssessmentReport } from './lib/assessment-deep-report.js';
...
try {
    ...
    const sources = ['deep-assessment.js', 'extended-reasoning.js', 'dynamic-chains.js', 'assessment.js', 'reasoning.js', 'advanced-techniques.js', 'chains.js', 'human-rating.js', 'techniques.js', 'solver.js', 'format.js',
        'scripts/assess-deep.js', 'scripts/lib/assessment-deep-report.js', 'scripts/lib/assessment-v2-report.js'];
    const sourceHashes = Object.fromEntries(sources.map(path => [path, hash(readFileSync(resolve(root, path)))]));
    const policy = { version: HUMAN_RATING_VERSION, profile: 'maintenance', limits: ANALYSIS_PROFILES.maintenance, techniqueOrder: HUMAN_TECHNIQUE_ORDER,
    ...
    if (single) {
        ...
        const records = selected.map((item, index) => {
            ...
            if (!['solved', 'unresolved', 'budget-exhausted'].includes(human.status)) throw new Error(`Invalid bank puzzle: ${item.difficulty}/${item.level} (${human.status})`);
            const production = assessV1(item.puzzle);
            ...
            return { ...item, sha256: hash(item.puzzle), searchNodes: SudokuSolver.rateDifficulty(item.puzzle),
                production: { solved: production.status === 'solved', placements: production.workload.placements }, human };
        });
        report = { schemaVersion: 1, policy,
